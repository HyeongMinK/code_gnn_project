...
def main(args, config, logger: Logger, run_id: int, datasetHelper: DatasetHelper, loaders, start_wall_time):
    ...
    for epoch in pbar:
        ...
        if epoch % config['eval_interval'] == 0:
            trn_labels, trn_probs, trn_preds = T.evaluation(datasetHelper,
                                                             train_loader,
                                                             model,
                                                             threshold_moving=config['threshold_moving'],
                                                             thres = config['thres'])
            trn_results = T.eval_model(trn_labels, trn_probs, trn_preds)
        ...
        #                 )
        now_monitor_val = getattr(dev_results, config['monitor'])
        now_trn_monitor = getattr(trn_results, config['monitor'])
        postfix_str = "[Epoch %d] Loss %.4f | Trn %s %.4f | Dev %s %.4f | Best [E%d] AUC %.4f GMean %.4f F1Ma %.4f AP %.4f RecMa %.4f" % (
                epoch, float(avg_train_loss), config['monitor'], now_trn_monitor, config['monitor'], now_monitor_val,
                best_metric_epoch, report_dev_res.auc_gnn, report_dev_res.gmean_gnn, report_dev_res.f1_macro, report_dev_res.ap_gnn, report_dev_res.recall_macro)
        if hasattr(loss_func, 'last_cls_loss'):
            postfix_str += "\n         └ cls=%.4f  rank=%.4f  rank_term=%.4f" % (
                loss_func.last_cls_loss, loss_func.last_rank_loss, loss_func.alpha * loss_func.last_rank_loss)
        tqdm.write(postfix_str)
        if config['test_each_epoch']:
            postfix_str_test = "         Test: AUC %.4f GMean %.4f F1Ma %.4f AP %.4f RecMa %.4f" % (
                report_test_res.auc_gnn, report_test_res.gmean_gnn, report_test_res.f1_macro, report_test_res.ap_gnn, report_test_res.recall_macro)
            tqdm.write(postfix_str_test)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_workers',      type = int,   default = None)   # yml fallback
    parser.add_argument('--seed',             type = int,   default = None)   # yml fallback
    ...
    parser.add_argument('--train_size',       type = float, default = None)   # yml fallback
    parser.add_argument('--val_size',         type = float, default = None)   # yml fallback
    parser.add_argument('--no_dev',         action = "store_true",  default = False)
    parser.add_argument('--gpu_id',           type = int,   default = None)   # yml fallback
    parser.add_argument('--multirun',         type = int,   default = None)   # yml fallback
    parser.add_argument('--model',            type = str,   default = 'LA-SAGE-S')
