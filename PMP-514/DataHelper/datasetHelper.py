...
class DatasetHelper(dataset):
    ...
    def load(self):
        ...
        elif self.dataset_name in ['tfinance', 'tsocial', 'yelpzip', 'yelpzip_emb', 'bridge', 'bridge_emb']:
        ...
        if self.config['model_name'] in ['LA-SAGE2', 'LA-SAGE-LI', 'LA-SAGE-S']:
    ...
    def config_data(self, data, dataset):
        ...
        self.val_nid         = torch.LongTensor(torch.nonzero(self.val_mask, as_tuple=True)[0])
        self.test_nid        = torch.LongTensor(torch.nonzero(self.test_mask, as_tuple=True)[0])
        self.num_classes     = dataset.num_classes if not self.dataset_name in ['tfinance', 'tsocial', 'grab', 'yelpzip', 'yelpzip_emb', 'bridge', 'bridge_emb'] else len(torch.unique(data.ndata['label']))
