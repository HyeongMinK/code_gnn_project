...
class LASAGESConv(nn.Module):
    def __init__(
        ...
        # lin_before_mp=True이면 fc_neigh가 먼저 in→out 투영하므로 집계 후 차원은 out_feats
        _agg_in_feats = out_feats if (self._in_src_feats > out_feats) else self._in_src_feats

        # self.fc_neigh_benign = nn.Linear(self._in_src_feats, out_feats, bias=False)
        self.fc_neigh_benign  = LIMLP(_agg_in_feats, hidden_channels=hid_feats, output_channels=out_feats,
        ...
        self.fc_neigh_fraud  = LIMLP(_agg_in_feats, hidden_channels=hid_feats, output_channels=out_feats,
                                     num_layers=num_trans, batch_size = batch_size, origin_infeat = origin_infeat, activation=mlp_activation)
